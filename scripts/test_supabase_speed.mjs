...
async function benchmark() {
  ...
  if (error) {
    ...
    console.log(`✅ [READ] Mengambil ${poles.length} tiang berhasil dalam: ${(t1 - t0).toFixed(2)} ms!`);
  }

  const t2 = performance.now();
  const testId = `BENCHMARK-${Date.now()}`;
  const { data: inserted, error: iErr } = await supabase.from('poles').insert({
    id: testId,
    pole_code: 'BENCH-001',
  ...
  if (iErr) {
    ...
    console.log(`✅ [WRITE/SAVE] Menyimpan 1 data tiang baru berhasil dalam: ${(t3 - t2).toFixed(2)} ms!`);
