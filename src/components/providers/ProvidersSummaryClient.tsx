...
function resolveSurveyorMeta(pole: Pole): SurveyorMeta {
  ...
  const known =
    ...
    KNOWN_SURVEYORS.find(
      (user) =>
        lookup.includes(user.displayName.toLowerCase().split(' ')[0]) &&
        lookup.includes(user.displayName.toLowerCase().split(' ').slice(-1)[0])
    ) ||
    (lookup.includes('tri')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-01')
      : undefined) ||
    (lookup.includes('frans')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-05')
      : undefined) ||
    (lookup.includes('fadlil')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-06')
      : undefined) ||
    (lookup.includes('rifqi')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-07')
      : undefined) ||
    (lookup.includes('yodi')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-02')
      : undefined) ||
    (lookup.includes('andika')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-03')
      : undefined) ||
    (lookup.includes('pradigga') || lookup.includes('pradiga')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-SURVEYOR-04')
      : undefined) ||
    (lookup.includes('admin') || lookup.includes('kominfo')
      ? KNOWN_SURVEYORS.find((user) => user.id === 'USR-KOMINFO-ADMIN')
      : undefined);
...
export default function ProvidersSummaryClient({
  ...
  const paginatedDaily = inputStats.daily.slice(
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-xs text-slate-300 leading-relaxed max-w-md">
                Rekap provider, kondisi aset, dan produktivitas input tim lapangan berdasarkan data
                live.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white/95 text-slate-900 rounded-3xl p-4 border border-white/70 shadow-[0_4px_20px_rgba(15,23,42,0.08)] min-h-[120px]">
              <div className="flex items-start justify-between gap-2">
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 leading-snug">
                  Total Aset Tiang
                </span>
                <Database className="w-4 h-4 text-blue-600 flex-shrink-0" />
              </div>
              <p className="mt-3 text-3xl font-black font-mono tracking-tight leading-none">
                {totalPolesCount}
              </p>
            ...
            <div className="bg-white/95 text-slate-900 rounded-3xl p-4 border border-white/70 shadow-[0_4px_20px_rgba(15,23,42,0.08)] min-h-[120px]">
              <div className="flex items-start justify-between gap-2">
                <span className="text-[10px] font-black uppercase tracking-wider text-sky-700 leading-snug">
                  Jaringan PLN
                </span>
            ...
            <div className="bg-white/95 text-slate-900 rounded-3xl p-4 border border-white/70 shadow-[0_4px_20px_rgba(15,23,42,0.08)] min-h-[120px]">
              <div className="flex items-start justify-between gap-2">
                <span className="text-[10px] font-black uppercase tracking-wider text-amber-700 leading-snug">
                  PJU Pemkot
                </span>
            ...
            <div className="bg-white/95 text-slate-900 rounded-3xl p-4 border border-white/70 shadow-[0_4px_20px_rgba(15,23,42,0.08)] min-h-[120px]">
              <div className="flex items-start justify-between gap-2">
                <span className="text-[10px] font-black uppercase tracking-wider text-indigo-700 leading-snug">
                  Provider ISP / FO
                </span>
                            ...
                            <span className="font-bold text-slate-800 block">
                              {st.hazardCount > 0 ? (
                                <span className="text-amber-700">
                                  ⚠️ {st.hazardCount} Tiang Masalah
                                </span>
                              ) : (
            ...
            <div className="bg-white rounded-3xl p-4 border border-slate-100 shadow-[0_2px_12px_rgba(15,23,42,0.04)] space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 leading-snug">
                    Tanggal aktif:{' '}
                    <strong className="text-slate-800">{selectedDayStats.date}</strong>. Kominfo:
                    Admin &amp; M. Tri. Bapenda: Yodi, Andika, Pradigga.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="col-span-2 rounded-2xl p-3.5 bg-slate-950 text-white">
                  <span className="text-[9px] uppercase font-black text-slate-300">
                    Total Semua
                  </span>
                ...
                <div className="rounded-2xl p-3 bg-blue-50 border border-blue-100 min-h-[96px]">
                  <span className="text-[9px] uppercase font-black text-blue-700">
                    Input Tanggal Ini
                  </span>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <p
                          className={`text-[10px] uppercase font-black ${isKominfo ? 'text-blue-700' : 'text-emerald-700'}`}
                        >
                          {team.label}
                        </p>
                      ...
                      <div
                        className={`w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 ${isKominfo ? 'bg-blue-600 text-white' : 'bg-emerald-600 text-white'}`}
                      >
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
                      ...
                      <div className="bg-white/75 rounded-2xl p-2 border border-white">
                        ...
                        <p className="font-black text-slate-900">
                          {formatPercent(selectedTeamTotal, selectedDayStats.total)}%
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-black text-slate-900">Filter Tanggal Input</h3>
                    ...
                    >
                      {inputStats.daily.map((d) => (
                        <option key={d.date} value={d.date}>
                          {d.date === todayJakarta ? `🌟 Hari Ini (${d.date})` : d.date} — {d.total}{' '}
                          tiang
                        </option>
                      ))}
                    </select>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-700">
                    ...
                    <strong className="text-blue-950 font-black">
                      {selectedDayStats.date === todayJakarta
                        ? `Hari Ini (${selectedDayStats.date})`
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    Rekap per petugas pada tanggal{' '}
                    <strong className="text-slate-700">{selectedDayStats.date}</strong>
                  </p>
                </div>
                            ...
                            {hasInput ? (
                              <div>
                                ...
                                <span className="text-[10px] text-slate-400 block font-bold">
                                  {formatPercent(user.selectedTotal, selectedDayStats.total)}%
                                  pangsa
                                </span>
                              </div>
                            ) : (
              ...
              {filteredUsers.length === 0 && (
                <div className="rounded-2xl bg-slate-50 border border-slate-100 p-6 text-center">
                  <p className="text-xs font-bold text-slate-500">
                    Tidak ada petugas yang cocok dengan filter tim atau pencarian &quot;
                    {userSearchQuery}&quot;.
                  </p>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1 space-y-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <h4 className="text-xs font-black text-slate-900 truncate">{prov.name}</h4>
                      </div>
